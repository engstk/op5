...
irqreturn_t smblib_handle_chg_state_change(int irq, void *data)
{
	...
	if (chg->need_suspend)
		vote(chg->usb_icl_votable, OP_CHG_DONE_VOTER, true, 0);
...
void ctrl_usb_suspend_function(bool status)
{
	pr_info("status=%d\n", status);
	g_chg->need_suspend = status;
	if (g_chg->need_suspend && g_chg->chg_done)
		vote(g_chg->usb_icl_votable, OP_CHG_DONE_VOTER, true, 0);
	else
		vote(g_chg->usb_icl_votable, OP_CHG_DONE_VOTER, false, 0);
}
/**************
